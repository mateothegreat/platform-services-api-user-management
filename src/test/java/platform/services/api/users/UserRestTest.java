...
@Log4j2
public class UserRestTest extends platform.services.api.users.BaseRestTests {
    ...
    void getBasePathWithAuthentication() {
        ...
        given().when()
               .auth()
               .basic(platform.services.api.users.UserConfig.USER_VALID_USERNAME, platform.services.api.users.UserConfig.USER_VALID_PASSWORD)
               .get()
               .then()
               .statusCode(200)
               .and()
    ...
    void getAll() {
        ...
        final Response response = given().when().auth().basic(platform.services.api.users.UserConfig.USER_VALID_USERNAME, platform.services.api.users.UserConfig.USER_VALID_PASSWORD).get("/users");
...
//                                        Authorities.ROLE_ADMIN, Authorities.ROLE_USER);
    ...
    void escalate_fail() {
        ...
        log.fatal(responseBody.prettyPeek());        response.then()
                .statusCode(403)
                .and()
                .contentType(ContentType.JSON);
